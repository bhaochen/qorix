...
from qorix.trainer.backends import TrainingBackend, build_lr_scheduler

if TYPE_CHECKING:
    from qorix.trainer.metrics.timeline import GPUTimelineLogger, _NullTracker
...
def _maybe_install_unified_memory_stub() -> bool:
    ...
    look stuck. Qorix's trainer path does not use Megatron inference contexts,
    ...
    - env QORIX_MEGATRON_DISABLE_UNIFIED_MEMORY_JIT (overrides config)
    """
    env_override = os.environ.get("QORIX_MEGATRON_DISABLE_UNIFIED_MEMORY_JIT")
    ...
    def _disabled_create_unified_mempool():
        raise RuntimeError("Unified memory mempool is disabled in qorix trainer.")
...
def _build_packed_segment_attention_mask_from_position_ids(
    ...
    Qorix can produce non-monotonic position_ids (multi-turn packed prefixes).
...
def _compute_rl_loss_vocab_parallel(
    ...
    with _track("loss_computation"):
        ...
        if config.cfg.seq_is_masking and vllm_delta is not None:
            from qorix.trainer.loss import _apply_seq_is_masking
        ...
        if algo == "cispo":
            ...
            from qorix.trainer.loss import _masked_mean_per_sample, _masked_count_per_sample
        ...
        if config.cfg.kl_penalty_tau > 0 and vllm_delta is not None:
            from qorix.trainer.loss import _compute_kl_penalty
    ...
    with _track("compute_kl"):
        ...
        if vllm_delta is not None:
            from qorix.trainer.loss import _compute_kl_from_logprob_diff
...
class MegatronBackend(TrainingBackend):
    ...
    def _loss_func(
        ...
        """
        from qorix.trainer.loss import compute_rl_loss
