...
Same pattern as ``qorix/utils/config_loader.py``:
...
from qorix.eval_standalone.config_schema import EvalStandaloneConfig
from qorix.utils.config_loader import (
    _coerce_value,
    _deep_merge,
    _set_dotpath,
)
...
def parse_args_and_load(argv: list[str] | None = None) -> EvalStandaloneConfig:
    ...
    parser = argparse.ArgumentParser(
        description="Qorix — standalone eval",
        add_help=True,
    )
    parser.add_argument(
        "--config",
