...
from qorix import table_schema_versions
from qorix.eval_standalone.config_schema import CheckpointEntry, EvalStandaloneConfig
...
def _install_compat_config(
    ...
    """Build a synthetic QorixConfig and install it as the global singleton.

    Existing modules (EvalRunner, generate.py, inference/server.py) all read
    from ``qorix.utils.config``.  Instead of refactoring them, we build a
