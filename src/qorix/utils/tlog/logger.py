"""
Professional logging system for Qorix.
...
class QorixFormatter(logging.Formatter):
    """
    Custom formatter for Qorix logs.
...
class DetailedFormatter(QorixFormatter):
...
class QorixLogger:
    """
    Logger wrapper with convenience methods for Qorix components.
...
def is_debug_mode() -> bool:
    """Return True if Qorix was started with --debug."""
    return _debug_mode
...
def _resolve_logs_path(logs_dir: Path | str | None) -> Path:
    ...
    if logs_dir is None:
        run_dir = os.environ.get("QORIX_RUN_DIR")
        if run_dir:
...
def setup_logging(
    ...
    console_handler.setFormatter(QorixFormatter(use_colors=True, console=True))
    root_logger.addHandler(console_handler)
    
    # Set up file handlers for each component
    for component in components:
        component_logger = logging.getLogger(f"qorix.{component}")
        ...
        standard_handler.setFormatter(QorixFormatter(use_colors=False))
        ...
        if component == "orchestrator":
            ...
            standard_buffered.setFormatter(QorixFormatter(use_colors=False))
...
def get_logger(component: str) -> QorixLogger:
    """
    ...
    Returns:
        QorixLogger instance for the component
    ...
    if component not in _loggers:
        ...
        logger = logging.getLogger(f"qorix.{component}")
        _loggers[component] = QorixLogger(logger, component)
