...
Usage from ``train.py``::
    ...
    cfg = parse_args_and_load()          # returns QorixConfig
...
def _resolve_flag(flag: str) -> str:
    ...
    - Validated against QorixConfig fields
    """
    canonical = flag.replace("-", "_").lower()
    if canonical in QorixConfig.model_fields:
        return canonical
...
def load_config(
    ...
    Returns a validated ``QorixConfig`` instance.
    """
    # Layer 1: defaults
...
def parse_args_and_load(argv: list[str] | None = None) -> QorixConfig:
    ...
    parser = argparse.ArgumentParser(
        description="Qorix — post-training with RL",
        add_help=True,
    )
    parser.add_argument(
        "--config",
