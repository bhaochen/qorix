...
from qorix.utils import config, paths
from qorix.utils.tlog import get_logger, setup_logging
from qorix.trainer.backends import create_backend
from qorix.trainer.metrics import GPUTimelineLogger, create_timeline_tracker
from qorix.trainer.metrics.torch_memory_logger import TorchMemoryLogger
from qorix.trainer.weight_sync import (
...
def _redirect_qorix_console_stream(stream: TextIO) -> None:
    """
    Point Qorix console handlers to the supplied stream.

    Keep file handlers untouched and only retarget console stream handlers.
    """
    root_logger = logging.getLogger("qorix")
...
def _build_default_ray_runtime_env(user_runtime_env: Any) -> dict[str, Any]:
    ...
    if bool(config.cfg.ray_propagate_run_dir):
        run_dir = os.environ.get("QORIX_RUN_DIR") or os.getcwd()
        env_vars.setdefault("QORIX_RUN_DIR", run_dir)
    ckpt_dir = os.environ.get("QORIX_CHECKPOINT_DIR")
    if ckpt_dir:
        env_vars.setdefault("QORIX_CHECKPOINT_DIR", ckpt_dir)
...
def resolve_worker_count(explicit_count: int | None, role: str) -> int:
    ...
    if explicit_count is None:
        raise ValueError(
            ...
            f"Please set {role.upper()}_NUM_WORKERS in `qorix.config`."
        )
...
def init_ray_cluster() -> dict[str, Any]:
    ...
    if not ray.is_initialized():
        from qorix.utils.tlog import is_debug_mode
        ...
        if runtime_env_hook and disable_runtime_env_hook:
            _log.warning(
                ...
                "Qorix launch to avoid per-worker uv env recreation."
            )
...
def _collect_node_setup_snapshot() -> dict[str, Any]:
    ...
    from qorix.orchestrator.loggers.system_info import get_system_info
...
class InferenceServerActor:
    ...
    def __init__(self, server_idx: int, bind_host: str = "0.0.0.0", model: str | None = None, config_data: dict | None = None):
        if config_data is not None:
            from qorix.utils.config import install_config
    ...
    def start(self, otlp_endpoint: str | None = None, startup_timeout_s: int = 600) -> dict[str, Any]:
        ...
        env = dict(os.environ)
        if otlp_endpoint:
            env["QORIX_OTLP_TRACES_ENDPOINT"] = otlp_endpoint
...
class TrainerRayActor:
    ...
    def _start_stdout_capture(self) -> None:
        ...
        _redirect_qorix_console_stream(self._stdout_tee_stream)
    ...
    def _stop_stdout_capture(self) -> None:
        ...
        if self._original_stdout is not None:
            _redirect_qorix_console_stream(self._original_stdout)
...
class RayInferenceGroup:
    ...
    def launch(self, otlp_endpoint: str | None = None) -> None:
        ...
        """
        from qorix.utils.config import get_config
