...
``qorix/evals/`` package.  Each subfolder must contain an ``eval.py``
with a concrete :class:`Eval` subclass.

Resolution order used by :func:`resolve`:
    1. Check for an eval folder under ``qorix/evals/<name>/eval.py``
    2. Fall back to ``qorix/environments/<name>/environment.py``
...
Usage::
    ...
    from qorix.evals import resolve, list_evals
...
from qorix.evals.base import Eval
...
def _load_eval_class(name: str) -> type[Eval]:
    """Import ``qorix.evals.<name>.eval`` and return the Eval subclass."""
    fqn = f"qorix.evals.{name}.eval"
    try:
...
def get_eval(name: str, **kwargs) -> Eval:
    ...
    The *name* must match a subfolder under ``qorix/evals/``
...
def resolve(name: str, **kwargs):
    ...
    Returns either an :class:`Eval` or an :class:`~qorix.environments.base.Environment`.
    """
    if name in list_evals():
        return get_eval(name, **kwargs)

    from qorix.environments import get_environment
