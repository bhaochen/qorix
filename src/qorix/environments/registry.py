...
``qorix/environments/`` package.  Each subfolder must contain an
...
from qorix.environments.base import Environment
...
def _load_env_class(name: str) -> type[Environment]:
    """Import ``qorix.environments.<name>.environment`` and return the Environment subclass."""
    ...
    fqn = f"qorix.environments.{name}.environment"
    try:
...
def get_environment(name: str, **kwargs) -> Environment:
    ...
    The *name* must match a subfolder under ``qorix/environments/``
