"""
hendrycks-math environment implementation.

Adapted from https://app.primeintellect.ai/dashboard/environments/primeintellect/hendrycks-math

Mathematical reasoning environment using the Hendrycks MATH dataset.
...
from qorix.environments.base import Sample, RewardResult, SingleTurnEnvironment
from qorix.environments.parsers import (
...
class HendrycksMathEnvironment(SingleTurnEnvironment):
    """
    Environment for Hendrycks MATH mathematical reasoning tasks.

    Uses the justus27/math-hendrycks-genesys-format dataset.
    ...
    def load_dataset(
        ...
        **kwargs
        ...
        """
        Load the Hendrycks MATH dataset.
        ...
        # Filter out invalid samples (None questions or answers)
        dataset = dataset.filter(
            lambda x: (
                isinstance(x.get("prompt"), str) and
                x.get("verification_info") is not None
            )
        )
        ...
        for item in dataset:
            ...
            # Extract ground truth answer from verification_info JSON
            verification_info = json.loads(item["verification_info"])
            answer = verification_info.get("ground_truth", "")
